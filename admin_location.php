<?php 
    ...
    $query = "SELECT unique_id, shop_name, address, email, status FROM users";
...
<body>
    ...
    // Add a custom icon for the user's location marker (green pin)
    ...
    sellers.forEach(function (seller) {
        // Check the role to determine the marker color
        var markerColor = (seller.role === 'seller') ? 'red' : 'blue';

        // Add a custom icon for each seller marker (red or blue pin)
        var marker = new google.maps.Marker({
            position: { lat: parseFloat(seller.lat), lng: parseFloat(seller.lng) },
            map: map,
            title: seller.shop_name,
            icon: {
                url: 'http://maps.google.com/mapfiles/ms/icons/' + markerColor + '-dot.png', // Set the URL of the custom icon
            }
        });

        markers.push(marker);

        // Add an info window for each seller marker
        var infowindow = new google.maps.InfoWindow({
            content: '<strong>' + seller.shop_name + '</strong><br>' + 'Address: ' + seller.address
        });

...
if(isset($_POST['save_student']))
{
    ...
    if($category == NULL || $name == NULL || $small_description == NULL || $price == NULL || $quantity == NULL || $status == NULL)
    {
        $res = [
            'status' => 422,
    ...
    if (in_array($img_extension, $extensions)) {
        ...
        if (move_uploaded_file($tmp_name, $destination)) {
            ...
            $query = "INSERT INTO product (category, name, small_description, price, quantity, status, image) VALUES ('$category', '$name', '$small_description', '$price', '$quantity', '$status', '$destination')";
...
{   
    ...
    $query = "UPDATE product SET category='$category',name='$name', small_description='$small_description', price='$price', quantity='$quantity', status='$status' 
                WHERE id='$student_id'";
...
if(isset($_GET['student_id'])) // Change 'student_id' to 'product_id'
{
    $product_id = mysqli_real_escape_string($conn, $_GET['student_id']); // Change $con to $conn

    $query = "SELECT * FROM product WHERE id='$product_id'"; // Change 'students' to 'products'
    $query_run = mysqli_query($conn, $query);
